                ...
                <el-radio-group v-model="radio1" size="medium" @change="changeShop">
                    <div v-for='(item, index) in sanplData' style='display: inline-block' :key='index'>
                        <el-tooltip placement="top" :content='item.offerId' :open-delay='openDelay'>
                            <el-radio-button :label="index">{{ $t('loan_detail_dp') }}{{ index + 1 }}({{
                                item.shopTypeName }})</el-radio-button>
                        </el-tooltip>
                    </div>
                        ...
                        <div class="score-info" style="width: 67%">
                            ...
                            <div class="score-num">
                                <span class="score" v-if="weekLineChartData.riskReportV3ShopScore">{{
                                    weekLineChartData.riskReportV3ShopScore.scoreSum || '0' }}</span>
                                <!-- <span class="history-score">
                                <i v-if="Number(itemData.currSellerScore - itemData.lastSellerScore) > 0" class="el-icon-top"></i>
                                <i v-else class="el-icon-bottom"></i>
                                {{ (itemData.currSellerScore - itemData.lastSellerScore).toFixed(2) }}
                            </span> -->
                            </div>
                            <div class="score-time"><span>{{ $t('loan_detail_gxsj') }}：{{ itemData.reportDate ||
                                '--' }}</span>
                    ...
                    <div class="no-amazon-left">
                        ...
                        <div class="no-amazon-content">
                            <el-descriptions :column="showOne">
                                ...
                                <el-descriptions-item :label="$t('loan_detail_dpofferid')">{{
                                    weekLineChartData.offerId || '--'
                                }}</el-descriptions-item>
                                ...
                                <el-descriptions-item :label="$t('loan_detail_zdxx')">{{
                                    sanplData[sanplDataIndex].site ? sanplData[sanplDataIndex].site : '--'
                                }}</el-descriptions-item>
                ...
                <el-card class="card-no-amazon">
                    <div class="no-amazon-left">
                        ...
                        <el-descriptions :column="showColum" class="sale-content">
                            <el-descriptions-item :label="$t('loan_detail_zjygyxs')">{{ itemData.month1SalesValue
                        ...
                        <div class="no-amazon-total">
                            {{ $t('loan_detail_jlgygmv') }}:
                            {{ sixTotalSales }}{{ $t('common_wy') }}
                        </div>
...
import { creditBeforeWeekData } from '../../../api/boss1';
export default {
    components: {
        xtable
    },
    ...
    watch: {
        ...
        sanplData: {
            ...
            handler(newV, oldV) {
                ...
                this.$nextTick(() => {
                    creditBeforeWeekData(this.$route.query.loanNumber).then((res) => {
    ...
    methods: {
        ...
        getWeekChantsNot(index) {
            ...
            this.$nextTick(() => {
                ...
                let myChartNo = this.$echarts.init(dom);
                myChartNo.setOption(this.weekChartsNotAmazonOptions);
        ...
        getWeekChants(index) {
            ...
            this.$nextTick(() => {
                ...
                this.myChart.setOption(this.weekChartsOptions);
        ...
        getOfferInfo(index) {
            ...
            this.tableData3[0].val6 = (this.itemData.salesRate * 100).toFixed(2) + '%';
            this.tableData3[0].val7 = (this.itemData.disbursementRate * 100).toFixed(2) + '%';
