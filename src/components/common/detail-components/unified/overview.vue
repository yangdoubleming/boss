<template>
    <div class="overview">
        <div style="display:flex; justify-content: space-between;">
            <el-card shadow="hover" class="card1">
                <div class="score-box">
                    <div class="score-info">
                        ...
                        <div class="score-time">{{ $t('loan_detail_gxsj') }}：{{ totalData.reportDate || '-' }}
                        </div>
                    </div>
            ...
            <el-card shadow="hover" class="card2" style="padding-left: 0px;">
                ...
                <div class="order-info">
                    <div class="money">
                        <span class="money-title">{{ $t('loan_detail_sxje') }}</span>
                        <span class="money-num" style="margin-bottom:40px">
                    ...
                    <div class="info">
                        <span class="info-item">{{ $t('home_zjffb') }}：{{ totalData.lenderName || '-' }}</span>
                        <span class="info-item">年利率：{{ totalData.interestRate || '-' }}</span>
                        <span class="info-item">还款方式：{{ totalData.repaymentMethod == 'EQUAL_PRINCIPAL_AND_INTEREST' ?
                            '等额本金' :
                            totalData.repaymentMethod == 'EQUAL_PRINCIPAL' ? '等额本息' : totalData.repaymentMethod ==
                                ...
                                ? '随借随还' : totalData.repaymentMethod == 'FLEXIBLE_REPAYMENT' ? '灵活还款' : '' ||
                                    '-' }}</span>
                        <span class="info-item">担保方：{{ totalData.gcName || '-' }}</span>
                        <span class="info-item">费率：{{ totalData.guaranteeRate || '-' }}</span>
                        <span class="info-item">费用类型：{{ totalData.gcpFeeType || '-' }}</span>
                        <span class="info-item">授信日期：{{ totalData.creditDate || '-' }}</span>
                        <span class="info-item">申请日期：{{ totalData.applicationTime || '-' }}</span>
                    </div>
                </div>
            </el-card>
        </div>
        ...
        <el-card shadow="hover" class="card3">
            ...
            <xtable :columns="columns" :detailTable="true" :tableData="totalData.applicationOfferVoList"
                @edit="shopInfoEvent" />
        </el-card>
        <el-card shadow="hover" class="card3">
            <div class="score-title" style="margin-bottom:36px">{{ $t('loan_detail_dkxx') }}</div>

            <xtable :columns="columns2" :detailTable="true" :tableData="totalData.loanVoList || []" />
        </el-card>
...
import { clearLoan, companyAll, loanSave, deleteLoan } from '../../../../api/index';
