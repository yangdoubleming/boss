<template>
	<div style="padding-bottom:100px;">
		<div style="background:#fff;padding:30px 20px;border-radius:0 20px 20px 20px">
			<div style="display:flex;align-items:center;margin-bottom:20px;justify-content: space-between;">
				...
				<div style="text-align:right;">
					...
					<el-button type="primary" icon="el-icon-printer" style="margin-left:30px" @click="_export">
						{{ $t('home_export') }}
					</el-button>
				</div>
			</div>
			<el-table v-loading="loading" :data="tableData" style="width: 100%" :border="true"
				...
				<el-table-column prop="head" :label="$t('home_xtime')" width="130px">
					<template slot-scope="scope">
						<div style="font-size:14px;text-align:center;height:30px;line-height:30px">{{ scope.row.head }}
						</div>
					</template>
				</el-table-column>
		...
		<div style="margin-top: 20px;position: relative;" id="c" v-loading="loading">
			<el-card style="border-radius:20px; position: relative;">
				<el-radio-group style="position: absolute;left: 220px;top: 65px;z-index: 999;" v-model="chartsCardType"
					@change="refurbishEcharts('')" size="25">
					<el-radio-button label="LOAN">{{ $t('home_azf') }}</el-radio-button>
					<el-radio-button label="CREADIT">{{ $t('home_acp') }}</el-radio-button>
					<el-radio-button label="LOANSUM">{{ $t('home_azfgs') }}</el-radio-button>
				</el-radio-group>
				<!--   <div
				...
				<div v-show="chartsCardType === 'LOAN'" style="margin-top: 50px">
					<div class="select-f" style="position: relative;">
						...
						<div style="float:right;padding-right:30px;">
							<div style="display:flex;">
								...
								<el-radio class="echarts-radio2" v-model="lenderLoanType" label="2" border size="medium">
					...
					<div style="position: absolute;top: 70px; right: 250px;" v-if="lenderLoanType == 1">
						<el-select class="loan-select" v-model="detail4Select" clearable
							@change="_changeOptionSelect(4, 1, 7)" placeholder="资方" multiple style="margin-right:30px">
							<el-option v-for="item in detail4SelectArr" :key="item.name" :label="item.name" clearable
								:value="item.name"></el-option>
						</el-select>
					</div>
					<div style="width:100%;display:flex; spa padding-top:20px">
						...
						<div style="width: 100%;display:flex; margin-top:60px" v-show="lenderLoanType == 2">
							<div id="detailChart4Pie" style="height: 520px; width:600px;!important"></div>
							<div class="charts-label-box">
								<div class="label-item" v-for="(item, index) in detail4Pie.series[0].data" :key="item.name">
									<div class="top">
										<div class="block"
				...
				<!-- 资金方授信金额 -->
				<div v-show="chartsCardType === 'LOAN'">
					<div class="select-f" style="position: relative;">
						{{ $t('home_sxje') }}
						<!--       <div style="float:right;padding-right:30px;">
				            <div style="display:flex;">
					...
					<div style="width:100%;display:flex;padding-top:20px">
						...
						<div style="width: 100%;display:flex;margin-top:60px" v-show="lenderLoanType == 2">
							...
							<div class="charts-label-box">
								<div class="label-item" v-for="(item, index) in detail1Pie.series[0].data" :key="item.name">
									<div class="top">
										<div class="block"
											:style="{ background: index + 1 % 12 != 0 ? colorArr[(index) % 12] : colorArr[11] }">
										</div>
				...
				<div v-show="chartsCardType === 'LOAN'" style="margin-top: 50px">
					...
					<div style="width:100%;display:flex;padding-top:20px">
						...
						<div style="width: 100%;display:flex;margin-top:60px" v-show="lenderLoanType == 2">
							...
							<div class="charts-label-box">
								<div class="label-item" v-for="(item, index) in detail7Pie.series[0].data" :key="item.name">
									<div class="top">
										<div class="block"
											:style="{ background: index + 1 % 12 != 0 ? colorArr[(index) % 12] : colorArr[11] }">
										</div>
				...
				<div v-show="chartsCardType === 'CREADIT'" style="margin-top: 50px">
					<div class="select-f" style="position: relative;">
						...
						<div style="float:right;padding-right:30px;">
							<div style="display:flex;">
								...
								<el-radio class="echarts-radio2" v-model="lenderLoanSumType" label="2" border size="medium">
									<img v-if="lenderLoanSumType == 2" src="../../assets/pie.png" alt />
									<img v-else src="../../assets/pie2.png" alt />
									{{ $t('home_bt') }}
								</el-radio>
							</div>
					...
					<div style="position: absolute;top: 70px; right: 250px;" v-if="lenderLoanSumType == 1">
						<el-select class="loan-select" v-model="detail5Select" clearable
							@change="_changeOptionSelect(5, 2, 8)" placeholder="产品" multiple style="margin-right:30px">
							<el-option v-for="item in detail5SelectArr" :key="item.name" :label="item.name" clearable
								:value="item.name"></el-option>
						</el-select>
					</div>
					<div style="width:100%;display:flex;padding-top:20px">
						...
						<div style="width: 100%;display:flex;margin-top:60px" v-show="lenderLoanSumType == 2">
							<div id="detailChart5Pie" style="height: 520px; width:600px;"></div>
							<div class="charts-label-box">
								<div class="label-item" v-for="(item, index) in detail5Pie.series[0].data" :key="item.name">
									<div class="top">
										<div class="block"
											:style="{ background: index + 1 % 12 != 0 ? colorArr[(index) % 12] : colorArr[11] }">
										</div>
				...
				<div v-show="chartsCardType === 'CREADIT'" style="margin-top: 50px">
					...
					<div style="width:100%;display:flex;padding-top:20px">
						...
						<div style="width: 100%;display:flex;margin-top:60px" v-show="lenderLoanSumType == 2">
							...
							<div class="charts-label-box">
								<div class="label-item" v-for="(item, index) in detail2Pie.series[0].data" :key="item.name">
									<div class="top">
										<div class="block"
											:style="{ background: index + 1 % 12 != 0 ? colorArr[(index) % 12] : colorArr[11] }">
										</div>
				...
				<div v-show="chartsCardType === 'CREADIT'" style="margin-top: 50px">
					...
					<div style="width:100%;display:flex;padding-top:20px">
						...
						<div style="width: 100%;display:flex;margin-top:60px" v-show="lenderLoanSumType == 2">
							...
							<div class="charts-label-box">
								<div class="label-item" v-for="(item, index) in detail8Pie.series[0].data" :key="item.name">
									<div class="top">
										<div class="block"
											:style="{ background: index + 1 % 12 != 0 ? colorArr[(index) % 12] : colorArr[11] }">
										</div>
				...
				<div v-show="chartsCardType === 'LOANSUM'" style="margin-top: 50px">
					<div class="select-f" style="position: relative;">
						...
						<div style="float:right;padding-right:30px;">
							<div style="display:flex;">
								...
								<el-radio class="echarts-radio2" v-model="paymentLoanType" label="2" border size="medium">
					...
					<div style="position: absolute;top: 70px; right: 250px;" v-if="paymentLoanType == 1">
						<el-select class="loan-select" v-model="detail6Select" clearable
							@change="_changeOptionSelect(6, 3, 9)" placeholder="支付公司" multiple style="margin-right:30px">
							<el-option v-for="item in detail6SelectArr" :key="item.name" :label="item.name" clearable
								:value="item.name"></el-option>
						</el-select>
					</div>
					<div style="width:100%;display:flex;padding-top:20px">
						...
						<div style="width: 100%;display:flex;margin-top:60px" v-show="paymentLoanType == 2">
							...
							<div class="charts-label-box">
								<div class="label-item" v-for="(item, index) in detail6Pie.series[0].data" :key="item.name">
									<div class="top">
										<div class="block"
											:style="{ background: index + 1 % 12 != 0 ? colorArr[(index) % 12] : colorArr[11] }">
										</div>
				...
				<div v-show="chartsCardType === 'LOANSUM'" style="margin-top: 50px">
					...
					<div style="width:100%;display:flex;padding-top:20px">
						...
						<div style="width: 100%;display:flex;margin-top:60px" v-show="paymentLoanType == 2">
							...
							<div class="charts-label-box">
								<div class="label-item" v-for="(item, index) in detail3Pie.series[0].data" :key="item.name">
									<div class="top">
										<div class="block"
											:style="{ background: index + 1 % 12 != 0 ? colorArr[(index) % 12] : colorArr[11] }">
										</div>
				...
				<div v-show="chartsCardType === 'LOANSUM'" style="margin-top: 50px">
					...
					<div style="width:100%;display:flex;padding-top:20px">
						...
						<div style="width: 100%;display:flex;margin-top:60px" v-show="paymentLoanType == 2">
							...
							<div class="charts-label-box">
								<div class="label-item" v-for="(item, index) in detail9Pie.series[0].data" :key="item.name">
									<div class="top">
										<div class="block"
											:style="{ background: index + 1 % 12 != 0 ? colorArr[(index) % 12] : colorArr[11] }">
										</div>
			...
			<el-card style="margin-top:20px;border-radius:20px"
				v-show="(lenderLoanType == 1 && chartsCardType == 'LOAN') || (paymentLoanType == 1 && chartsCardType == 'LOANSUM') || (lenderLoanSumType == 1 && chartsCardType == 'CREADIT')">
...
export default {
	...
	mounted() {
		boardDate('MONTH').then(res => {
			...
			res.data.loanLenderAmountMap[`loanLenderAmount${Object.keys(res.data.loanLenderAmountMap).length}`]
				.forEach(item => {
					if (item.loanAmount >= 5000000) {
						this.showLender.push(item.lenderName || '其他');
					} else {
						this.showLender.push('其他');
					}
				});
			res.data.loanLenderProductAmountMap[
				...
				if (item.loanAmount >= 5000000) {
					...
					this.showProduct.push('其他');
				}
			});
			res.data.loanPaymentAmountMap[
				`loanPaymentAmountMap${Object.keys(res.data.loanPaymentAmountMap).length}`].forEach(
					item => {
						if (item.loanAmount >= 5000000) {
							this.showPayment.push(item.paymentName || '其他');
						} else {
							this.showPayment.push('其他');
	...
	methods: {
		getUserSumOrOfferSum(dates) {
			boardPolyline(dates[0], dates[1], this.detailTabIndex).then(res => {
				...
				Object.keys(res.data.userRegisterCountMap).forEach((keys, index) => {
					this.detailOption5.series[0].data.push(res.data.userRegisterCountMap[
						`userRegisterCount${index + 1}`]);
				});
				Object.keys(res.data.userCreditCountMap).forEach((keys, index) => {
					this.detailOption5.series[1].data.push(res.data.userCreditCountMap[
						`userCreditCount${index + 1}`]);
				});
				Object.keys(res.data.userLoanCountMap).forEach((keys, index) => {
					this.detailOption5.series[2].data.push(res.data.userLoanCountMap[
						`userLoanCount${index + 1}`]);
				});
				Object.keys(res.data.userCreditRateMap).forEach((keys, index) => {
					this.detailOption5.series[3].data.push(res.data.userCreditRateMap[
						`userCreditRate${index + 1}`]);
				});
				Object.keys(res.data.userLoanRateMap).forEach((keys, index) => {
					this.detailOption5.series[4].data.push(res.data.userLoanRateMap[
						`userLoanRate${index + 1}`]);
				});
				...
				Object.keys(res.data.authorizeOfferCountMap).forEach((keys, index) => {
					this.detailOption6.series[0].data.push(res.data.authorizeOfferCountMap[
						`authorizeOfferCount${index + 1}`]);
				});
				Object.keys(res.data.creditShopCountMap).forEach((keys, index) => {
					this.detailOption6.series[1].data.push(res.data.creditShopCountMap[
						`creditShopCount${index + 1}`]);
				});
				Object.keys(res.data.loanShopCountMap).forEach((keys, index) => {
					this.detailOption6.series[2].data.push(res.data.loanShopCountMap[
						`loanShopCount${index + 1}`]);
				});
				Object.keys(res.data.creditShopRateMap).forEach((keys, index) => {
					this.detailOption6.series[3].data.push(res.data.creditShopRateMap[
						`creditShopRate${index + 1}`]);
				});
				Object.keys(res.data.loanShopRateMap).forEach((keys, index) => {
					this.detailOption6.series[4].data.push(res.data.loanShopRateMap[
						`loanShopRate${index + 1}`]);
				});
		...
		refurbishEcharts(chartsIndexone, chartsIndextwo, chartsIndexthree) {
			if (chartsIndextwo == 1 || !chartsIndextwo) {
			...
			myChart1Pie.resize(resize);

			if (chartsIndextwo == 2 || !chartsIndextwo) {
			...
			myChart2Pie.resize(resize);

			if (chartsIndextwo == 3 || !chartsIndextwo) {
			...
			myChart3Pie.resize(resize);


			if (chartsIndexone == 4 || !chartsIndexone) {
			...
			myChart4Pie.resize(resize);

			if (chartsIndexone == 5 || !chartsIndexone) {
			...
			myChart5Pie.resize(resize);


			if (chartsIndexone == 6 || !chartsIndexone) {
			...
			myChart6Pie.resize(resize);


			if (chartsIndexthree == 7 || !chartsIndexthree) {
			...
			myChart7Pie.resize(resize);


			if (chartsIndexthree == 8 || !chartsIndexthree) {
			...
			myChart8Pie.resize(resize);


			if (chartsIndexthree == 9 || !chartsIndexthree) {
		...
		sumItem(arr1, arr2) {
			let newArr = arr1.map(function (item, index) {
				return Number(item) + Number(arr2[index]);
			});
			return newArr;
		},

		_changeOptionSelect(chartsIndexone, chartsIndextwo, chartsIndexthree) {

			if (this.detailTabIndex == 'MONTH' && this.termSeven[0].length < 8) {
				this.termSeven = [this.termSeven[0] + '-01', this.termSeven[1] + '-01'];
			}
			this.getChartsData(this.termSeven, chartsIndexone, chartsIndextwo, chartsIndexthree);
			var myEvent = new Event('resize');
			window.dispatchEvent(myEvent);
		},
		getChartsData(date, chartsIndexone, chartsIndextwo, chartsIndexthree) {
			boardColumnar(date[0], date[1], this.detailTabIndex).then(res => {
				...
				this.detail1.series = []; //总的数据
				this.detail1.xAxis[0].data = dates; //横坐标
				this.detail1.legend.data = []; //下面点击的选项
				...
				this.detail1.legend.data.push('其他');
				...
				Object.keys(res.data.creditLenderAmountMap).forEach((key, index) => {
					res.data.creditLenderAmountMap[`creditLenderAmount${index + 1}`].forEach(item => {
						this.detail1.series.forEach(it => {
							if (it.name == item.lenderName || (!item.lenderName && it
								.name == '其他')) {
				...
				this.detail1.series.forEach((it, ind) => {
					if (this.showLender.indexOf(it.name) != -1) {
						...
						if (ShowView1) {
							...
							dataIndex1 = filterData1.findIndex((item) => {
								return item.name == "其他"
							})
							filterData1[dataIndex1].data = this.sumItem(filterData1[dataIndex1].data,
								it.data)
						}
					}
				})
				filterData1[dataIndex1].data.forEach(sum => {
					total1 = Number(sum) + total1;
				})
				...
				filterData1.forEach(it => {
					...
					this.detail1Pie.series[0].data.unshift({
						name: it.name,
						value: itTotal,
						scale: ((itTotal / total1) * 100).toFixed(2)
					});
				...
				// if (qita1) {
				// 	this.detail1Pie.series[0].data.push({
				// 		name: '其他',
				// 		value: qita1,
				// 		scale: ((qita1 / total1) * 100).toFixed(2)
				// 	});
				// }
				filterData1 = filterData1.filter(item => {
					let tot = 0
				...
				if (this.detail4Select.length) {
					...
					let arr31 = []
					this.detail4Select.forEach(item => {
						this.detail1Pie.series[0].data.forEach((it) => {
							if (item == it.name) {
								arr31.push(it);
							}
						})
					})
					this.detail1Pie.series[0].data = arr31
				}
				...
				this.detail2.legend.data.push('其他');


				Object.keys(res.data.creditLenderProductAmountMap).forEach((key, index) => {
					res.data.creditLenderProductAmountMap[`creditLenderProductAmount${index + 1}`]
						.forEach(item => {
							this.detail2.series.forEach(it => {
								if (it.name == item.lenderProduct || (!item
									.lenderProduct && it.name == '其他')) {
									it.data.push((item.creditAmount / 10000).toFixed(2));
								}
							});
						});
				...
				this.detail2.series.forEach((it, ind) => {
					if (this.showProduct.indexOf(it.name) != -1) {
						...
						if (ShowView2) {
							...
							dataIndex2 = filterData2.findIndex((item) => {
								return item.name == "其他"
							})
							filterData2[dataIndex2].data = this.sumItem(filterData2[dataIndex2].data,
								it.data)
						}
					}

				});
				filterData2[dataIndex2].data.forEach(sum => {
					total2 = Number(sum) + total2;
				})
				...
				filterData2.forEach(it => {
					...
					this.detail2Pie.series[0].data.unshift({
						name: it.name,
						value: itTotal2,
						scale: ((itTotal2 / total2) * 100).toFixed(2)
					});
					// }
				});
				...
				// if (qita2) {
				// 	this.detail2Pie.series[0].data.push({
				// 		name: '其他',
				// 		value: qita2,
				// 		scale: ((qita2 / total2) * 100).toFixed(2)
				// 	});
				// }
				filterData2 = filterData2.filter(item => {
					let tot = 0
					item.data.forEach(it => {
						tot = Number(it) + tot
					})
				...
				if (this.detail5Select.length) {
					...
					let arr35 = []
					this.detail5Select.forEach(item => {
						this.detail2Pie.series[0].data.forEach((it) => {
							if (item == it.name) {
								arr35.push(it);
							}
						})
					})
					this.detail2Pie.series[0].data = arr35
				}
				...
				this.detail3.legend.data.push('其他');


				Object.keys(res.data.creditPaymentAmountMap).forEach((key, index) => {
					res.data.creditPaymentAmountMap[`creditPaymentAmount${index + 1}`].forEach(
						item => {
							this.detail3.series.forEach(it => {
								if (it.name == item.paymentName || (!item.paymentName && it
									.name == '其他')) {
									it.data.push((item.creditAmount / 10000).toFixed(2));
								}
							});
				...
				this.detail3.series.forEach((it, ind) => {
					if (this.showPayment.indexOf(it.name) != -1) {
						...
						if (ShowView3) {
							...
							dataIndex3 = filterData3.findIndex((item) => {
								return item.name == "其他"
							})
							filterData3[dataIndex3].data = this.sumItem(filterData3[dataIndex3].data,
								it.data)
						}
					}

				});
				filterData3[dataIndex3].data.forEach(sum => {
					total3 = Number(sum) + total3;
				})
				...
				filterData3.forEach(it => {
					...
					this.detail3Pie.series[0].data.unshift({
						name: it.name,
						value: itTotal3,
						scale: ((itTotal3 / total3) * 100).toFixed(2)
					});
					// }
				});
				...
				// if (qita3) {
				// 	this.detail3Pie.series[0].data.push({
				// 		name: '其他',
				// 		value: qita3,
				// 		scale: ((qita3 / total3) * 100).toFixed(2)
				// 	});
				// }
				filterData3 = filterData3.filter(item => {
					let tot = 0
					item.data.forEach(it => {
						tot = Number(it) + tot
					})
				...
				if (this.detail6Select.length) {
					...
					let arr38 = []
					this.detail6Select.forEach(item => {
						this.detail3Pie.series[0].data.forEach((it) => {
							if (item == it.name) {
								arr38.push(it);
							}
						})
					})
					this.detail3Pie.series[0].data = arr38
				}
				...
				this.detail4.legend.data.push('其他');
				...
				this.detail4.series.forEach((it, ind) => {
					if (this.showLender.indexOf(it.name) != -1) {
						...
						if (ShowView4) {
							...
							dataIndex4 = filterData4.findIndex((item) => {
								return item.name == "其他"
							})
							filterData4[dataIndex4].data = this.sumItem(filterData4[dataIndex4].data,
								it.data)
						}
					}

				});
				filterData4[dataIndex4].data.forEach(sum => {
					total4 = Number(sum) + total4;
				})
				...
				filterData4.forEach(it => {
					...
					this.detail4Pie.series[0].data.unshift({
						name: it.name,
						value: itTotal4,
						scale: ((itTotal4 / total4) * 100).toFixed(2)
					});
					// }
				...
				// if (qita4) {
				// 	this.detail4Pie.series[0].data.push({
				// 		name: '其他',
				// 		value: qita4,
				// 		scale: ((qita4 / total4) * 100).toFixed(2)
				// 	});
				// }
				filterData4 = filterData4.filter(item => {
					let tot = 0
					item.data.forEach(it => {
						tot = Number(it) + tot
					})
				...
				if (this.detail4Select.length) {
					...
					let arr30 = []
					this.detail4Select.forEach(item => {
						this.detail4Pie.series[0].data.forEach((it) => {
							if (item == it.name) {
								arr30.push(it);
							}
						})
					})
					this.detail4Pie.series[0].data = arr30
				}
				...
				this.detail5.legend.data.push('其他');


				Object.keys(res.data.loanLenderProductAmountMap).forEach((key, index) => {
					res.data.loanLenderProductAmountMap[`loanLenderProductAmount${index + 1}`].forEach(
						item => {
				...
				this.detail5.series.forEach((it, ind) => {
					if (this.showProduct.indexOf(it.name) != -1) {
						...
						if (ShowView5) {
							...
							dataIndex5 = filterData5.findIndex((item) => {
								return item.name == "其他"
							})
							filterData5[dataIndex5].data = this.sumItem(filterData5[dataIndex5].data,
								it.data)
						}
					}
				...
				filterData5[dataIndex5].data.forEach(sum => {
					total5 = Number(sum) + total5;
				})
				...
				filterData5.forEach(it => {
					...
					this.detail5Pie.series[0].data.unshift({
						name: it.name,
						value: itTotal5,
						scale: ((itTotal5 / total5) * 100).toFixed(2)
					});
					// }
				});
				...
				// if (qita5) {
				// 	this.detail5Pie.series[0].data.push({
				// 		name: '其他',
				// 		value: qita5,
				// 		scale: ((qita5 / total5) * 100).toFixed(2)
				// 	});
				// }
				filterData5 = filterData5.filter(item => {
					let tot = 0
					item.data.forEach(it => {
						tot = Number(it) + tot
					})
				...
				if (this.detail5Select.length) {
					...
					let arr34 = []
					this.detail5Select.forEach(item => {
						this.detail5Pie.series[0].data.forEach((it) => {
							if (item == it.name) {
								arr34.push(it);
							}
						})
					})
					this.detail5Pie.series[0].data = arr34
				}
				...
				this.detail6.legend.data.push('其他');
				...
				this.detail6.series.forEach((it, ind) => {
					if (this.showPayment.indexOf(it.name) != -1) {
						...
						if (ShowView6) {
							...
							dataIndex6 = filterData6.findIndex((item) => {
								return item.name == "其他"
							})
							filterData6[dataIndex6].data = this.sumItem(filterData6[dataIndex6].data,
								it.data)
						}
					}

				});
				filterData6[dataIndex6].data.forEach(sum => {
					total6 = Number(sum) + total6;
				})
				...
				filterData6.forEach(it => {
					...
					this.detail6Pie.series[0].data.unshift({
						name: it.name,
						value: itTotal6,
						scale: ((itTotal6 / total6) * 100).toFixed(2)
					});
					// }
				});
				...
				// if (qita6) {
				// 	this.detail6Pie.series[0].data.push({
				// 		name: '其他',
				// 		value: qita6,
				// 		scale: ((qita6 / total6) * 100).toFixed(2)
				// 	});
				// }
				filterData6 = filterData6.filter(item => {
					let tot = 0
					item.data.forEach(it => {
						tot = Number(it) + tot
					})
				...
				if (this.detail6Select.length) {
					...
					let arr37 = []
					this.detail6Select.forEach(item => {
						this.detail6Pie.series[0].data.forEach((it) => {
							if (item == it.name) {
								arr37.push(it);
							}
						})
					})
					this.detail6Pie.series[0].data = arr37
				}
				...
				this.detail7.legend.data.push('其他');

				Object.keys(res.data.loanOfferLenderCountMap).forEach((key, index) => {
					res.data.loanOfferLenderCountMap[`loanOfferLenderCount${index + 1}`].forEach((
						item) => {
				...
				this.detail7.series.forEach((it, ind) => {
					if (this.showLender.indexOf(it.name) != -1) {
						...
						if (ShowView7) {
							...
							dataIndex7 = filterData7.findIndex((item) => {
								return item.name == "其他"
							})
							filterData7[dataIndex7].data = this.sumItem(filterData7[dataIndex7].data,
								it.data)
						}
					}
				...
				filterData7[dataIndex7].data.forEach(sum => {
					total7 = Number(sum) + total7;
				})
				...
				filterData7.forEach(it => {
					...
					this.detail7Pie.series[0].data.unshift({
						name: it.name,
						value: itTotal7,
						scale: ((itTotal7 / total7) * 100).toFixed(2)
					});
					// }
				});
				...
				// if (qita7) {
				// 	this.detail7Pie.series[0].data.push({
				// 		name: '其他',
				// 		value: qita7,
				// 		scale: ((qita7 / total7) * 100).toFixed(2)
				// 	});
				// }
				filterData7 = filterData7.filter(item => {
					let tot = 0
					item.data.forEach(it => {
						tot = Number(it) + tot
					})
				...
				if (this.detail4Select.length) {
					...
					let arr32 = []
					this.detail4Select.forEach(item => {
						this.detail7Pie.series[0].data.forEach((it) => {
							if (item == it.name) {
								arr32.push(it);
							}
						})
					})
					this.detail7Pie.series[0].data = arr32
				}
				...
				this.detail8.legend.data.push('其他');

				Object.keys(res.data.loanOfferLenderProductCountMap).forEach((key, index) => {
					res.data.loanOfferLenderProductCountMap[`loanOfferLenderProductCount${index + 1}`]
						.forEach((item) => {
				...
				this.detail8.series.forEach((it, ind) => {
					if (this.showProduct.indexOf(it.name) != -1) {
						...
						if (ShowView8) {
							...
							dataIndex8 = filterData8.findIndex((item) => {
								return item.name == "其他"
							})
							filterData8[dataIndex8].data = this.sumItem(filterData8[dataIndex8].data,
								it.data)
						}
					}
				...
				filterData8[dataIndex8].data.forEach(sum => {
					total8 = Number(sum) + total8;
				})
				...
				filterData8.forEach(it => {
					...
					this.detail8Pie.series[0].data.unshift({
						name: it.name,
						value: itTotal8,
						scale: ((itTotal8 / total8) * 100).toFixed(2)
					});
					// }
				});
				...
				// if (qita8) {
				// 	this.detail8Pie.series[0].data.push({
				// 		name: '其他',
				// 		value: qita8,
				// 		scale: ((qita8 / total8) * 100).toFixed(2)
				// 	});
				// }
				filterData8 = filterData8.filter(item => {
					let tot = 0
					item.data.forEach(it => {
						tot = Number(it) + tot
					})
				...
				if (this.detail5Select.length) {
					...
					let arr36 = []
					this.detail5Select.forEach(item => {
						this.detail8Pie.series[0].data.forEach((it) => {
							if (item == it.name) {
								arr36.push(it);
							}
						})
					})
					this.detail8Pie.series[0].data = arr36
				}
				...
				this.detail9.legend.data.push('其他');


				Object.keys(res.data.loanOfferPaymentCountMap).forEach((key, index) => {
					res.data.loanOfferPaymentCountMap[`loanOfferPaymentCount${index + 1}`].forEach(
						item => {
				...
				this.detail9.series.forEach((it, ind) => {
					if (this.showPayment.indexOf(it.name) != -1) {
						...
						if (ShowView9) {
							...
							dataIndex9 = filterData9.findIndex((item) => {
								return item.name == "其他"
							})
							filterData9[dataIndex9].data = this.sumItem(filterData9[dataIndex9].data,
								it.data)
						}
					}

				});
				filterData9[dataIndex9].data.forEach(sum => {
					total9 = Number(sum) + total9;
				})
				...
				filterData9.forEach(it => {
					...
					this.detail9Pie.series[0].data.unshift({
						name: it.name,
						value: itTotal9,
						scale: ((itTotal9 / total9) * 100).toFixed(2)
					});
					// }
				});
				...
				// if (qita9) {
				// 	this.detail9Pie.series[0].data.push({
				// 		name: '其他',
				// 		value: qita9,
				// 		scale: ((qita9 / total9) * 100).toFixed(2)
				// 	});
				// }
				filterData9 = filterData9.filter(item => {
					let tot = 0
					item.data.forEach(it => {
						tot = Number(it) + tot
					})
				...
				if (this.detail6Select.length) {
					...
					let arr39 = []
					this.detail6Select.forEach(item => {
						this.detail9Pie.series[0].data.forEach((it) => {
							if (item == it.name) {
								arr39.push(it);
							}
						})
					})
					this.detail9Pie.series[0].data = arr39
				}
				this.refurbishEcharts(chartsIndexone, chartsIndextwo, chartsIndexthree);
			});
		},
		getTabelData(dates) {
			// dates是日期
			...
			boardTrend(dates[0], dates[1], this.detailTabIndex).then(res => {
				...
				if (this.detailTabIndex == 'MONTH') {
					Object.keys(res.data.dateMap).forEach((key, index) => {
						this.colunm.push({
							label: res.data.dateMap[`month${index + 1}`],
							key: `val${index}`
						});
					});
				} else if (this.detailTabIndex == 'WEEK') {
					Object.keys(res.data.dateMap).forEach((key, index) => {
						this.colunm.push({
							label: res.data.dateMap[`week${index + 1}`],
							key: `val${index}`
						});
					});
				} else {
					Object.keys(res.data.dateMap).forEach((key, index) => {
						this.colunm.push({
							label: res.data.dateMap[`day${index + 1}`],
							key: `val${index}`
						});
					});
				}
				Object.keys(res.data.creditAmountMap).forEach((key, index) => {
					this.tableData[0][`val${index}`] = (res.data.creditAmountMap[
						`creditAmount${index + 1}`] / 10000).toFixed(2);
				});
				Object.keys(res.data.loanAmountMap).forEach((key, index) => {
					this.tableData[1][`val${index}`] = (res.data.loanAmountMap[
						`loanAmount${index + 1}`] / 10000).toFixed(2);
				});
				Object.keys(res.data.loanRateMap).forEach((key, index) => {
					this.tableData[2][`val${index}`] = (res.data.loanRateMap[`loanRate${index + 1}`] *
						100).toFixed(2) + '%';
				});
				Object.keys(res.data.creditUserCountMap).forEach((key, index) => {
					this.tableData[3][`val${index}`] = res.data.creditUserCountMap[
						`creditUserCount${index + 1}`];
				});
				Object.keys(res.data.loanSellerCountMap).forEach((key, index) => {
					this.tableData[4][`val${index}`] = res.data.loanSellerCountMap[
						`loanSellerCount${index + 1}`];
				});
				Object.keys(res.data.loanSellerRateMap).forEach((key, index) => {
					this.tableData[5][`val${index}`] =
						this.handleCutZero(((res.data.loanSellerRateMap[
							`loanSellerRate${index + 1}`] || 0) * 100).toFixed(2)) + '%';
					//
				});
			});
		},
		...
		lastYear() {
			...
			data.setMonth(data.getMonth() - data.getMonth(), 1); //获取到当前月份,设置月份
