<template>
    <div v-if="showPage">
                ...
                <el-card shadow="hover" :body-style="{ padding: '0px', height: '60px' }">
                    <div class="grid-content grid-con-1" style="display: flex">
                        <el-menu :default-active="activeIndex" class="el-menu-demo" style="width: 92%" mode="horizontal"
                            ...
                            <el-menu-item index="5">{{ $t('loan_detail_sqxx') }}</el-menu-item>
                            <el-menu-item index="10">支付公司回传信息</el-menu-item>
        ...
        <div v-loading="checkTabLoading">
            <!-- <overview
                :pageData="pageData"
                @toAchievement="toAchievement"
                :shopreportData="shopreportData"
                v-if="activeIndex == 1"
                @refreshDetail="_getDetail"
                @toLoanning="_toLoanning"
            /> -->
            ...
            <loanedPayCompanyInfo v-if="activeIndex == 10" :pageData="payCompanyInfo"></loanedPayCompanyInfo>
            <shopInfo :pageData="shopData" v-show="activeIndex == 5" @refreshDetail="_getDetail"
                :amazonData="amazonData" @openCreditInfo="dialogFormVisible = true" />
...
import shopInfo from '../../common/detail-components/loaned-shopInfo.vue';
import loaning from '../../common/detail-components/loaning.vue';
import pfDataModel from '../../common/detail-components/pfDataModel.vue';
import loanedPayCompanyInfo from '../../common/detail-components/loaned-payCompanyInfo.vue';
...
import { creditBefore, creditAfter, credits, getPfDataModel, shopreport, getPayCompanyInfo } from '../../../api/boss1';

export default {
    name: '',
    components: {
        loanedPayCompanyInfo,
        overview,
        applyinfo,
        achievementData,
        PFscoring,
    ...
    methods: {
        ...
        _getDetail() {
            getPayCompanyInfo(this.$route.query.loanCode).then(res => {
                this.payCompanyInfo = res.data
            })
            //{{$t('loan_detail_gl') }}
            companyAll().then((res) => {
                this.companyList = res.data;
                shopreport(this.$route.query.loanNumber).then((re) => {
                    this.shopreportData = re.data.riskReportShopScore || {};
                });
            });
            ...
            shopInfoF(this.$route.query.loanNumber).then((res) => {
                res.data.forEach((item) => {
                    ...
                    item.paymentLockStatus = item.paymentLockStatus == 'DOUBLE_LOCK' ? '双锁' : item.paymentLockStatus == 'LOCK' ? '单锁' : '无锁'
                });
        ...
        handleSelect(key, keypath) {
            ...
            if (this.activeIndex == 3) {
                ...
                creditBefore(this.$route.query.loanNumber)
                    .then((res) => {
                        res.data.riskReports.forEach((item) => {
                            item.offerDataSetInfo.offerId = item.offerId;
                        });
                        this.sanplData = res.data.riskReports;
