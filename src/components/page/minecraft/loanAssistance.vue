<template>
    <div>
        <div style="display: flex; align-items: center; padding: 18px">
            ...
            <span style="color: rgba(0, 0, 0, 0.65); font-size: 14px; padding-left: 5px">{{ $t('loan_detai_rmbsqlb') }}</span>
        </div>
                    ...
                    <el-col :span="6">
                        <div style="display: flex; align-items: flex-end; height: 100px; width: 100%; justify-content: flex-end">
                            <el-button
                                ...
                                v-if="$showBtn('CREATE_APPLICATION_ASS')"
                                >{{ $t('loan_add_new') }}</el-button
                            >
                        </div>
                    </el-col>
                </el-row>
        ...
        <el-drawer size="62%" :title="$t('loan_detail_xjsq')" :visible.sync="dialogFormVisible" direction="rtl">
            ...
            >
                ...
                <div>
                    <el-form-item :label="$t('loan_detail_sfzzm')" style="margin-right: 60x">
                        <el-upload class="avatar-uploader" :action="uploadUrl" :show-file-list="false" :on-success="handleAvatarSuccess1">
                            <img v-if="form.bCardFrontPath" :src="imageUrl + form.bCardFrontPath" class="avatar" />
                            <i v-else class="el-icon-plus avatar-uploader-icon"></i>
                        </el-upload>
                    </el-form-item>
                    <el-form-item :label="$t('loan_detail_sfzfm')" style="margin-left: 72px">
                        <el-upload class="avatar-uploader" :action="uploadUrl" :show-file-list="false" :on-success="handleAvatarSuccess2">
                            <img v-if="form.bCardBackPath" :src="imageUrl + form.bCardBackPath" class="avatar" />
                            <i v-else class="el-icon-plus avatar-uploader-icon"></i>
                        </el-upload>
                    </el-form-item>
                </div>
                ...
                <el-form-item :label="$t('home_zjffb')" style="margin-right: 30px" prop="lenderId">
                    <el-select v-model="form.lenderId" :placeholder="$t('loan_detail_choose')" @change="changeLender">
                        <el-option :label="item.label" :value="item.value" v-for="(item, index) in applyLender" :key="index"></el-option>
                    </el-select>
                </el-form-item>
                ...
                <el-form-item :label="$t('home_zfgs')" style="margin-right: 30px" prop="paymentCode">
                    <el-select v-model="form.paymentCode" :placeholder="$t('loan_detail_choose')">
                        <el-option
                            ...
                            v-for="(item, index) in payCompany"
                            :key="index"
                        ></el-option>
                    </el-select>
                </el-form-item>
            ...
            <div class="dialog-footer" style="text-align: center; margin-top: 50px">
                ...
                <el-button type="primary" @click="_newOrder" icon="el-icon-document-checked" style="">{{ $t('common_submit') }}</el-button>
    ...
    data() {
        return {
            ...
            columns: [
                ...
                {
                    label: this.$t('loan_detail_khbg'),
                    prop: 'customerReportTxt',
                    color: '#fe1964',
                    width: 100
                },
                {
        ...
        onExport() {
            ...
            axios({
                ...
                url: this.filterOptions[6].value.length
                    ? `${this.baseUrl}/lending/application/export?applicationStatus=${this.applicationStatus}&applicationCode=${this.filterOptions[1].value}&bName=${this.filterOptions[2].value}&bPhoneNumber=${this.filterOptions[3].value}&lenderId=${this.filterOptions[5].value}&applicationTimeStart=${this.filterOptions[6].value[0]}&applicationTimeEnd=${this.filterOptions[6].value[1]}`
                    : `${this.baseUrl}/lending/application/export?applicationStatus=${this.applicationStatus}&applicationCode=${this.filterOptions[1].value}&bName=${this.filterOptions[2].value}&bPhoneNumber=${this.filterOptions[3].value}&lenderId=${this.filterOptions[5].value}`,
                headers: {
                ...
                if (res.data.records) {
                    res.data.records.forEach((item) => {
                        ...
                        item.applicationStatus = this.orderStatus.indexOf(item.applicationStatus).toString()
                            ...
                            : '';
                        item.customerReport ? (item.customerReportTxt = '查看报告') : (item.customerReportTxt = '上传报告');
                    });
                }
