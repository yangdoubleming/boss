<template>
  <div v-if="showPage">
          ...
          <div class="title-box" style="align-items: center">
            ...
            <div class="right">
              <el-button type="success" @click="pushOrder"
                v-if="applicationStatus === '已创建' && $showBtn('SUBMIT_LOAN')">{{ $t('common_submit') }}</el-button>
              <el-button type="success" @click="_received1st"
                v-if="applicationStatus === '已提交' && $showBtn('RECEIVED1ST')">{{ $t('loan_detail_chtg') }}</el-button>
              <el-button type="success" @click="_received2nd"
                v-if="applicationStatus === '已初核' && $showBtn('RECEIVED2ST_ASS')">{{ $t('loan_detail_fhtg')
                }}</el-button>
              <el-button type="success" @click="_openApprove"
                v-if="applicationStatus === '已复核' && $showBtn('EXAMINE')">{{ $t('loan_detail_sp') }}</el-button>
              <el-button type="warning" @click="dialogFormVisible2 = true"
                v-if="applicationStatus === '已批准' && $showBtn('CREATE_LOAN_ASS_D')">{{ $t('loan_detail_cjdk')
                }}</el-button>
        ...
        <el-card shadow="hover" :body-style="{ padding: '0px', height: '60px' }">
          <div class="grid-content grid-con-1" style="display: flex">
            <el-menu :default-active="activeIndex" class="el-menu-demo" style="width: 92%" mode="horizontal"
              ...
              <el-menu-item index="1" v-if="!['25', '48', '59'].includes($route.query.lenderId)">{{ $t('loan_detail_gl')
                }}</el-menu-item>
              <el-menu-item index="2">{{ $t('loan_detail_sqxx') }}</el-menu-item>
              <el-menu-item index="5" v-if="!['25', '48', '59'].includes($route.query.lenderId)">{{
    $t('loan_detail_dpxx') }}</el-menu-item>
              <el-menu-item index="3" v-if="!['25', '48', '59'].includes($route.query.lenderId)">{{
    $t('loan_detail_sxbg') }}</el-menu-item>
              <el-menu-item index="6" v-if="!['25', '48', '59'].includes($route.query.lenderId)">{{
    $t('loan_detail_jkbg') }}</el-menu-item>
              <el-menu-item index="7" v-if="pageData.lenderName.includes('浦发')">{{ $t('loan_detail_zfbg')
                }}</el-menu-item>
              <el-menu-item index="8" v-if="$route.query.lenderId === '25'">{{ $t('loan_detail_xpdp') }}</el-menu-item>
              <el-menu-item index="9" v-if="$route.query.lenderId === '28' ||
    ...
    ">{{ $t('loan_detail_fkzb') }}</el-menu-item>
              ...
              <el-menu-item index="11" v-if="!['48', '59'].includes($route.query.lenderId)">{{ $t('loan_detail_bfbk')
                }}</el-menu-item>
              <el-menu-item index="12" v-if="['57'].includes($route.query.lenderId)">{{ $t('loan_detail_shxx')
                }}</el-menu-item>
    ...
    <div v-loading="checkTabLoading">
      ...
      <shopInfo :pageData="shopData" :creditAmount="pageData.creditAmount" v-if="activeIndex == 5"
        ...
        @openCreditInfo="dialogFormVisible = true" />
    ...
    <el-dialog title="授信信息" :visible.sync="dialogFormVisible" width="40%">
      <el-form :inline="true" :model="form" :rules="rules" ref="form" style="padding-left: 20px" label-width="150px"
        ...
        <el-form-item label="授信时间" style="margin-right: 30px" prop="dates">
          <el-date-picker v-model="form.dates" type="datetimerange" range-separator="至" start-placeholder="开始日期"
    ...
    <el-dialog title="创建贷款" :visible.sync="dialogFormVisible2" width="50%" v-loading="loanLoading">
      <el-form :inline="true" :model="form2" :rules="rules2" ref="form2" style="padding-left: 20px" label-width="150px"
        label-position="left">
        ...
        <el-form-item label="贷款产品" style="margin-right: 30px" prop="lenderProductId">
          <el-select v-model="form2.lenderProductId" :placeholder="$t('loan_detail_choose')" clearable
            style="width: 200px">
            <el-option v-for="item in titData.product" :key="item.id" :label="item.productName"
