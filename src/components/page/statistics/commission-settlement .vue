    ...
    <div>
                ...
                <el-card shadow="hover" :body-style="{ padding: '0px' }">
                    <div class="grid-content grid-con-1">
                        <div class="grid-cont-right">
                            ...
                            <span class="grid-num2">{{ loanData.userCount }}</span>
                        </div>
                        <div class="grid-cont-right">
                            <span class="grid-num">贷款总返佣</span>
                            <span class="grid-num2">{{ loanData.totalRebateAmount }}{{ $t('common_yuan2') }}</span>
                        </div>
                        <div class="grid-cont-right">
                            <span class="grid-num">待结算贷款佣金</span>
                            <span class="grid-num2">{{ loanData.beSettlementAmount }}{{ $t('common_yuan2') }}</span>
                        </div>

                        <div class="grid-cont-right">
                            <span class="grid-num">待生效贷款佣金</span>
                            <span class="grid-num2">{{ loanData.beEffectiveAmount }}{{ $t('common_yuan2') }}</span>
                        </div>
                        <div class="grid-cont-right">
                            <span class="grid-num">已结算贷款佣金</span>
                            <span class="grid-num2">{{ loanData.settlementAmount }}{{ $t('common_yuan2') }}</span>
                        </div>
                        <div class="grid-cont-right">
                            <span class="grid-num">上月待结算贷款佣金</span>
                            <span class="grid-num2">{{ loanData.lastMonthBeSettlementAmount
                                }}{{ $t('common_yuan2') }}</span>
                        </div>
                        <div class="grid-cont-right">
                            <span class="grid-num">可结算贷款佣金</span>
                            <span class="grid-num2" style="color: #409EFF">{{ loanData.settlableAmount
                                }}{{ $t('common_yuan2') }}</span>
                        </div>
                    </div>
                </el-card>
        ...
        <el-row :gutter="24" class="dataList" style="margin-left: 0; margin-right: 0">
            ...
            <el-card shadow="hover" :body-style="{ padding: '0px' }">
                <el-table ref="multipleTable" :row-key="getRowKey" :data="tableData"
                    ...
                    tooltip-effect="dark" @sort-change="handleSortChange" @select="handleSelectionChange"
                    ...
                    <el-table-column prop="beEffectiveAmount" label="待生效金额/元" align="center" sortable="custom">
                    </el-table-column>
        ...
        <el-dialog title="批量结算佣金" :visible.sync="dialogVisible" width="30%" class="dialogClass"
            ...
            <div style="
                ...
                <p>
                    已选 <span style="color: #e9335e">{{ multipleSelection.length }}</span> 个合伙人，可结算佣金共计
                    <span style="color: #e9335e">{{ totalAmount }}</span> 元
                </p>
                <p style="font-weight: 900; font-size: 14px; padding-top: 15px">批量处理为【已结算】</p>
            </div>
            <span slot="footer" class="dialog-footer">
                <div style="display: flex; justify-content: center; align-items: center">
                    <el-button @click="handleClose"
                        style="width: 100px; color: #409EFF; margin-right: 15px; border-color: #409EFF">取 消</el-button>
                    <el-button type="primary" @click="_push" style="width: 100px" :disabled="disabledSettle">确
                        定</el-button>
                </div>
...
<script>
import { getStatisticsAmount, getLoanRegisterAmountList, settleable, batchUserLiquidation } from '@/api/insur';
...
export default {
    ...
    created() {
        getStatisticsAmount('', '', 3).then((res) => {
            if (res.code === '200') {
    ...
    methods: {
        async handleSelectAll(selection) {
            if ((selection.length && !this.checked) || (!selection.length && this.checked)) {
                ...
                await getLoanRegisterAmountList({
        ...
        searchData() {
            this.ruleForm.dateStart = this.dateTime[0];
            this.ruleForm.dateEnd = this.dateTime[1];
        ...
        exportList() {
            ...
            axios({
                method: 'post',
                responseType: 'blob',
                url: `${this.TEXT_URL}/download/exportLoanCommission`,
                headers: {
                ...
                .then(function (res) {
                    ...
                    downloadElement.download = `贷款佣金结算${moment().format('YYYYMMDD')}.xls`;
