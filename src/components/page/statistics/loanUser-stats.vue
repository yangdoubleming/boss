...
<script>
import { userWeeks } from '../../../api/index';
export default {
    name: 'loanUser-stats',
    data() {
        return {
    ...
    methods: {
        ...
        getData() {
            ...
            userWeeks(this.weekNumber).then((res) => {
                res.data.forEach((item) => {
                    this.option5.series[0].data.unshift(item.currentUserCount);
                    this.option5.series[1].data.unshift(item.userCount);
                    this.option5.xAxis[0].data.unshift(item.lastSaturday + '-' + item.thisFriday);

                    this.option.series[0].data.unshift(item.currentActiveUserCount);
                    this.option.series[1].data.unshift(item.activeUserCount);
