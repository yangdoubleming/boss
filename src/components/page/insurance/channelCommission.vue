        ...
        <el-row :gutter="24" class="dataList" style="margin-left: 0; margin-right: 0">
            ...
            <el-card shadow="hover" :body-style="{ padding: '0px' }">
                ...
                >
                    ...
                    <el-table-column prop="channelType" label="渠道分类" align="center" width="135px"></el-table-column>
                    <el-table-column prop="channelName" label="渠道名" align="center" width="135px"></el-table-column>
                    <el-table-column prop="channelCode" label="渠道码" align="center" width="135px"></el-table-column>
...
<script>
import { channelCommissionList, platformList2 } from '@/api/insur';
...
export default {
    ...
    methods: {
        ...
        getDataList(paramer) {
            channelCommissionList(paramer).then(res => {
                // console.log(res.data);
                if (res.code === 200) {
                    this.tableData = res.data.records;
                    this.total = res.data.total;
                    this.policyAmout =res.data.records.length? res.data.records[0].totalMoney : 0
                    res.data.records.forEach(i => {
                        this.options.forEach(item => {
                            i.channelType = i.channelType == item.platformCode ? item.platfromName : i.channelType;
                        });
                        i.moneyInterestRate = (Number(i.moneyInterestRate) * 100).toFixed(2) + '%';
        ...
        exportExcel() {
            ...
            axios({
                method: 'get',
                responseType: 'blob',
                url: `${this.baseUrl}/channel-market/report/merchant/checkOrderExcel?channelType=${this.ruleForm.channelType}&channelCode=${this.ruleForm.channelCode}&orderNo=${this.ruleForm.orderNo}&rewardRulesId=${this.ruleForm.rewardRulesId}&payStartTime=${this.ruleForm.payStartTime}&payEndTime=${this.ruleForm.payEndTime}&orderStartTime=${this.ruleForm.orderStartTime}&orderEndTime=${this.ruleForm.orderEndTime}`,
                headers: {
                ...
                .then(function(res) {
                    ...
                    if (that.ruleForm.placeTime && that.ruleForm.placeEnd) {
                        downloadElement.download = `${that.ruleForm.placeTime}-${that.ruleForm.placeEnd}渠道返佣流水表.xlsx`;
                    } else {
                        downloadElement.download = `渠道返佣流水表.xlsx`;
                    }
