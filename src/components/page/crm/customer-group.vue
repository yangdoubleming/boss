<template>
    <div v-loading="TabalLoaidng">
        ...
        <el-card shadow="hover" :body-style="{ padding: '0px' }">
            <el-row :gutter="24" class="searchForm">
                <el-col :span="24">
                    <el-form ref="ruleForm" :model="ruleForm" style="padding: 30px 20px 30px 20px">
                        ...
                        <el-row class="searchForm" style="margin-top: 20px">
                            <el-form-item>
                                <el-select v-model="ruleForm.sourceTypeId" placeholder="客户来源">
                                    <el-option :style="{ width: '140px' }" v-for="item in optionsSourceTypeIdList"
                        ...
                        <el-row>
                            <div class="button" style="display: flex; align-items: center; gap: 15px;margin-top: 10px;">
                                <div>已选中：{{ multipleSelection.length }}项</div>
                                <el-button type="primary" v-if="$showBtn('transferOtherOwnerA')"  @click="_settlement">分配至他人</el-button>
                                <el-button type="primary" v-if="$showBtn('crmListCreateCustomer')" @click="_addNewCustomer">新建客户</el-button>
                                <el-button type="primary" @click="_ownerSearch"
                                    v-if="$showBtn('ownershipStatus')">归属查询</el-button>
                                <el-button type="primary" v-if="$showBtn('transferOtherOwner')" @click="_changeToLam">转LAM跟进</el-button>
                            </div>
                        </el-row>
                    </el-form>
                </el-col>
            ...
            <el-row :gutter="24" class="dataList" style="margin-left: 0; margin-right: 0">
                <el-table ref="multipleTable" :row-key="getRowKey" :data="tableData"
                    ...
                    <el-table-column prop="intentionalDegree" label="意向程度" align="center">
                        <template slot-scope="scope">
                            {{ formatDegree(scope.row.intentionalDegree) || '--' }}
                        </template></el-table-column>
                    <el-table-column prop="customItem140C" label="客户成熟度" align="center">
                        <template slot-scope="scope">
                            {{ formatCustomItem140C(scope.row.customItem140C) || '--' }}
                        </template></el-table-column>
                    ...
                    <el-table-column prop="action" label="操作" align="center" width="200px" fixed="right">
                        <template slot-scope="scope">
                            <el-button v-if="$showBtn('crmListSeeDetails')" @click="showDetail(scope.row)" type="text">查看详情</el-button>
                            <el-button v-if="$showBtn('crmListFollowRecords')" @click="followRecord(scope.row)" type="text">跟进记录</el-button>
                            <el-button @click="liberatePoolCustomer(scope.row)" type="text">释放</el-button>
                        </template>
                    </el-table-column>
                </el-table>
        ...
        <el-drawer v-loading="drawerLoaidng" :visible.sync="drawerVisible" direction="rtl"
            :before-close="handleDrawerClose" class="drawerFrom">

            <div slot="title" class="drawer-title">
                <div class="customer-name">{{ currentAccountInfo.borrowerName }}</div>
                <el-button type="primary" @click="EditCustomer">编辑</el-button>

            </div>
            <div class="drawer-main">
                <div class="drawer-card drawer-grid">
                    <div>
                        <div>注册手机号</div>
                        <div class="detail-content">{{ customerPhone || '--' }}</div>
                    </div>
                    <div>
                        <div>客户所有人</div>
                        <div class="detail-content">{{ currentAccountInfo.owerName }}</div>
                    </div>
                    <div class="grid-item">
                        <div>支付公司</div>
                        <div class="detail-content">{{ currentAccountInfo.payment }}</div>
                    </div>
                    <div class="grid-item">
                        <div>客户成熟度</div>
                        <div class="detail-content">{{ currentAccountInfo.type }}</div>
                    </div>
                    <div class="grid-item">
                        <div>平台+站点</div>
                        <div class="detail-content">{{ currentAccountInfo.platform }}</div>
                    </div>
                </div>
                ...
                <div class="drawer-card">
                    ...
                    <div style="padding: 20px"
                        v-if="currentAccountInfo.customerHistoryList && currentAccountInfo.customerHistoryList.length > 0">
                ...
                <div class="drawer-card">
                    <div>备注</div>
                    <div>{{ currentAccountInfo.remark || '--' }}</div>
                </div>
            </div>
        </el-drawer>
        <el-dialog :title="isEdit ? '编辑客户' : '新增客户'" :visible.sync="addMoreVisible" width="40%"
            :before-close="resetAdd">
            <el-form label-position="right" :model="customerForm" ref="customerRef" :rules="customerRules"
                label-width="120px">
                <el-row :gutter="4">
                    <el-col :span="11">
                        <el-form-item label="客户名称" prop="name" v-if="!isEdit">
                            <el-input v-model="customerForm.name" placeholder="请输入客户名称"></el-input>
                        </el-form-item>
                        <el-form-item label="客户名称" v-else>
                            <el-input v-model="customerForm.accountName" placeholder="请输入客户名称"></el-input>
                        </el-form-item>
                    </el-col>
                    <el-col :span="11">
                        <el-form-item label="注册手机号" prop="phone">
                            <el-input v-model="customerForm.phone" placeholder="请输入注册手机号"></el-input>
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row :gutter="4">
                    <el-col :span="11">
                        <el-form-item label="平台+站点" prop="platform">
                            <el-input type="textarea" maxlength="50" show-word-limit v-model="customerForm.platform"
                                placeholder="请输入平台+站点"></el-input>
                        </el-form-item>
                    </el-col>
                    <el-col :span="11">
                        <el-form-item label="支付公司" prop="payment">
                            <el-input v-model="customerForm.payment" placeholder="请输入支付公司"></el-input>
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row :gutter="4">
                    <el-col :span="11">
                        <el-form-item label="客户成熟度" prop="type">
                            <el-select v-model="customerForm.type" placeholder="请选择客户成熟度">
                                <el-option label="A-符合进件条件,近1个月有资金需求" value="A-符合进件条件,近1个月有资金需求"></el-option>
                                <el-option label="B-符合进件条件,3个月内(短期)有资金需求" value="B-符合进件条件,3个月内(短期)有资金需求"></el-option>
                                <el-option label="C-符合进件条件,没有明确的资金需求" value="C-符合进件条件,没有明确的资金需求"></el-option>
                                <el-option label="D-不符合进件条件,3个月内能达到进件要求" value="D-不符合进件条件,3个月内能达到进件要求"></el-option>
                                <el-option label="E-不符合进件条件,无资金需求(需备注客户经营平台)"
                                    value="E-不符合进件条件,无资金需求(需备注客户经营平台)"></el-option>
                                <el-option label="F-还需进一步开发,未能拿到客户的信息及贷款意愿"
                                    value="F-还需进一步开发,未能拿到客户的信息及贷款意愿"></el-option>
                                <el-option label="G-无法联系(手机号无效、微信无法添加、3次以上联系无效)"
                                    value="G-无法联系(手机号无效、微信无法添加、3次以上联系无效)"></el-option>
                            </el-select>
                        </el-form-item>
                    </el-col>
                    <el-col :span="11">
                        <el-form-item label="资金需求" prop="capital">
                            <el-input v-model="customerForm.capital" placeholder="请输入资金需求"></el-input>
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row :gutter="4">
                    <el-col :span="11">
                        <el-form-item label="客户所有人" v-if="isEdit">
                            <el-input v-model="customerForm.owerName" :disabled="true"></el-input>
                        </el-form-item>
                        <el-form-item label="客户所有人" v-else>
                            <el-input v-model="loginUser" :disabled="true"></el-input>
                        </el-form-item>
            ...
            <div slot="footer" class="dialog-footer">
                <el-button @click="resetAdd" style="width: 80px; height: 35px">取 消</el-button>
                <el-button type="primary" @click="ensureAdd('customerRef')" style="width: 80px; height: 35px">确
                    定</el-button>
            </div>
        </el-dialog>
        <!-- 跟进记录抽屉 -->
        ...
        <!-- 归属查询弹窗 -->
        <el-dialog title="归属查询" :visible.sync="ownerSearchDialogVisible" width="30%"
            :before-close="handleCloseOwnerSearch" :close-on-click-modal="false">
            <div class="main" v-loading="loadingOwnerSearch">
                <div class="pay_methods">
                    <div class="filter-operation-box">
                        <el-input v-model="customerPhone" placeholder="请输入客户名称或MID"></el-input>
                    </div>
                    <el-descriptions title="" :column="1" v-if="info.message">
                        <el-descriptions-item label="归属状态">{{ info.status == 'TRUE' ? '已归属' : '未归属'
                        }}</el-descriptions-item>
                        <el-descriptions-item label="详情">{{ info.message }}</el-descriptions-item>
                        <el-descriptions-item label="mid">{{ info.mid ? info.mid : '-' }}</el-descriptions-item>
                    </el-descriptions>
                    <el-button style="width: 120px; margin:0 auto;margin-top: 60px;display: block;" type="primary"
                        @click="blurList">查询</el-button>
                </div>
            </div>
        </el-dialog>
        <!-- 转LAM跟进 -->
        <el-dialog title="转LAM跟进" :visible.sync="lamDialogVisible" width="30%" :before-close="handleCloseLam"
            :close-on-click-modal="false">
            <div class="main">
                <el-select v-model="lamer" placeholder="请选择其他归属人" style="width: 80%;">
                    <el-option v-for="item in optionsLam" :key="item.id" :label="item.merchantName" :value="item.id">
                    </el-option>
                </el-select>
                <el-button @click="sureLamer" style="width: 120px; margin:0 auto;margin-top: 60px;display: block;" type="primary">确定</el-button>
            </div>
        </el-dialog>
    </div>
</template>
...
import { getFollow, saveFollow, accountInfoOwn, listByGroup, listGroupPage, transferOwner, updateAccount, addOwner, editOwner, getSourceByLevel, getLabelList, ownershipStatus, changeToLam, getThreeSource, liberatePoolCustomer } from '@/api/crm'; //导入文件
export default {
    ...
    data() {
        return {
            lamer:'',
            optionsLam: [],
            lamDialogVisible: false,
            customerPhone: '',
            info: {},
            loadingOwnerSearch: false,
            ownerSearchDialogVisible: false,
            optionsSourceTypeIdList: [],
            optionsLabelIds: [],
            customerPhone: "",
            intentionalShow: {
                0: '无意向',
    ...
    methods: {
        ...
        liberatePoolCustomer(item) {
            this.$alert(`确定释放【${item.accountName}】？`, '释放', {
        ...
        getSourceByLevel() {
            getSourceByLevel({
                ...
                this.optionsSourceTypeIdList = res.data
            })
        },
        // 新建跟进记录
        ...
        ensureAdd(formName) {
            this.$refs[formName].validate(valid => {
                if (valid) {
                    if (!this.isEdit) {
                        addOwner({ ...this.customerForm, "loginId": this.loginUser }).then((res) => {
                            if (res.code == 200) {
                        ...
                        editOwner({ ...this.customerForm, "loginId": this.loginUser }).then((res) => {
                            if (res.code == 200) {
        ...
        openDrawer(row) {
            this.drawerLoaidng = true
            this.drawerVisible = true;
            this.currentAccountId = row.id;
            this.customerPhone = row.phone
            accountInfoOwn({ account: row.phone }).then((res) => {
                this.customerForm = { ...res.data, accountName: res.data.borrowerName }
                this.showName = this.customerForm.borrowerName
        ...
        async handleSelectAll(selection) {
            if ((selection.length && !this.checked) || (!selection.length && this.checked)) {
                // 全选
                this.loading = true;
                await listGroupPage({ ...this.ruleForm, "loginId": this.loginUser }).then((res) => {
                    if (res.code === 200) {
        ...
        _changeToLam() {
            if (!this.multipleSelection.length) return this.$message.info('请先勾选！');
            this.lamDialogVisible = true
            this.TabalLoaidng = true
            getThreeSource({ threeSourceId: '11111221' }).then((res) => {
                this.TabalLoaidng = false
                this.optionsLam = res.data
                // 显示弹窗
                this.lamDialogVisible = true
            }).catch(() => {
                this.TabalLoaidng = false
        ...
        // 确定转LAM跟进
        sureLamer(){
            if (!this.lamer) return this.$message.warning('请选择其他归属人');
            changeToLam({ 
                neocrmAccountIds: this.multipleSelection.map(item => item.id),
                merchantId: this.lamer,
             }).then((res) => {
                this.$message.success('操作成功！');
                // 关闭弹窗
                this.lamDialogVisible = false
                // 查询列表
                this.getDataList();
            })
        },
        ...
        // 归属查询
        _ownerSearch() {
            this.ownerSearchDialogVisible = true
        },
        blurList() {
            let obj = {}
            if (this.customerPhone) {
                ...
                return this.$message.warning('请输入客户名称或MID')
            }
            this.loadingOwnerSearch = true
            ownershipStatus(obj).then((res) => {
                console.log("🚀 ~ ownershipStatus ~ res:", res)
                this.info = res.data
                this.loadingOwnerSearch = false
            }).catch((err) => {
                this.info = {}
                this.loadingOwnerSearch = false
        ...
        handleCloseOwnerSearch() {
            this.ownerSearchDialogVisible = false
            this.info = {}
            this.customerPhone = ''
        },
        searchData() {
            console.log(55556666, this.ruleForm.labelIds);
            if (this.timeArray.length) {
                this.ruleForm.startTime = this.timeArray[0];
                this.ruleForm.endTime = this.timeArray[1];
            } else {
            ...
            // this.total = 0;
            this.currentPage = 1;
            if (this.ruleForm.labelIds.length > 0) {
                this.ruleForm.labelIds = this.ruleForm.labelIds.join(',')
            }
            this.getDataList();
        },
        getDataList() {
            this.TabalLoaidng = true;
            listGroupPage({ ...this.ruleForm, "loginId": this.loginUser }).then((res) => {
                ...
                // 逗号还原成数组
                if (this.ruleForm.labelIds) {
                    this.ruleForm.labelIds = this.ruleForm.labelIds.split(',')
                    this.ruleForm.labelIds = this.ruleForm.labelIds.map(Number)
                }
                this.TabalLoaidng = false;
            }).catch(error => {
                this.TabalLoaidng = false;
        ...
        formatDegree(val){
          return  this.intentionalShow[val]
        },
        formatCustomItem140C(val){
            return  this.maturity[val]
        },
        getAmount() {
            //重要代码
