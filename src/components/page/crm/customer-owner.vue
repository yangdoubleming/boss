...
import { getFollow, saveFollow, accountInfoOwn, listByGroup, listOwnerPage, transferOwner, updateAccount, addOwner, editOwner, getSourceByLevel, getLabelList, ownershipStatus, changeToLam, getThreeSource, liberatePoolCustomer } from '@/api/crm'; //导入文件
export default {
    ...
    methods: {
        ...
        async handleSelectAll(selection) {
            if ((selection.length && !this.checked) || (!selection.length && this.checked)) {
                // 全选
                this.loading = true;
                await listOwnerPage({ ...this.ruleForm, "loginId": this.loginUser }).then((res) => {
                    if (res.code === 200) {
        ...
        getDataList() {
            this.TabalLoaidng = true;
            listOwnerPage({ ...this.ruleForm, "loginId": this.loginUser }).then((res) => {
