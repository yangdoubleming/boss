<template>
    <div>
        <div style="display: flex; align-items: center; padding: 18px">
            ...
            <span style="color: rgba(0, 0, 0, 0.65); font-size: 14px; padding-left: 5px"> / 贷前管理 / 黑名单列表</span>
        </div>
        <div class="main-content">
            <el-form :model="ruleForm" ref="ruleForm" label-width="50px" class="demo-ruleForm" style="margin-top: 20px">
                ...
                <el-row>
                    ...
                    <el-col :span="5">
                        <el-form-item>
                            <el-select v-model="ruleForm.productSign" multiple collapse-tags clearable placeholder="适用产品" style="width: 100%">
                                <el-option v-for="item in productSignOptions" :key="item.value" :label="item.label" :value="item.value"></el-option>
                            </el-select>
                        </el-form-item>
                    </el-col>
                    <el-col :span="6" >
            ...
            <NewTable @page-change="handlePageChange" :current="ruleForm.current" :totalSize="totalSize" :columns="columns"
                      :tableData="tableData" style="margin-top: 20px">
                <template v-slot:productSign>
                    <el-table-column label="适用产品" prop="productSign" align="center" width="120">
                        <template slot-scope="scope">
                            {{ formatProductSign(scope.row.productSign) }}
                        </template>
                    </el-table-column>
                </template>
        ...
        <el-dialog class="createDialog"  :modal-append-to-body="false" title="新增黑名单客户"  :visible.sync="createDialogVisible" width="600px" center>
            <div>
                <el-form ref="addForm" :rules="addRule" label-width="150px" :model="addForm">
                    ...
                    <el-row>
                        <el-col :span="20">
                            <el-form-item label="自定义黑名单规则:" prop="rule">
                    ...
                    <el-row>
                        <el-col :span="20">
                            <el-form-item label="适用产品:" prop="productSign">
                                <el-select v-model="addForm.productSign" multiple collapse-tags clearable placeholder="请选择" style="width: 100%">
                                    <el-option v-for="item in productSignOptions" :key="item.value" :label="item.label" :value="item.value"></el-option>
                                </el-select>
                            </el-form-item>
                        </el-col>
                ...
                <div slot="footer" class="dialog-footer">
                    ...
                    <el-button type="primary" @click="submitAdd('addForm')">新增</el-button>
                </div>
            </div>
        </el-dialog>
    </div>
...
import { getAddBlackList, getBlackList, getStopBlackList } from '@/api/risk';
export default {
    name: 'black-list',
    mixins: [tableMixin],
    ...
    data() {
        return {
            ...
            addRule: {
                ...
                productSign: [
                    { required: false, message: '请选择适用产品', trigger: 'change' }
                ],
                status: [
    ...
    methods: {
        ...
        buildQueryParams() {
            ...
            const ps = params.productSign;
            if (Array.isArray(ps)) {
                params.productSign = ps.length ? ps.join(',') : undefined;
            }
            if (params.productSign === '') params.productSign = undefined;
            return params;
        },
        buildAddParams() {
            const data = { ...this.addForm };
            const ps = data.productSign;
            if (Array.isArray(ps)) {
                data.productSign = ps.length ? ps.join(',') : undefined;
            }
            if (data.productSign === '') data.productSign = undefined;
            return data;
        },
        toAdd(){
        ...
        _getList() {
            getBlackList(this.buildQueryParams()).then(res=>{
        ...
        submitAdd(formName){
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    getAddBlackList(this.buildAddParams()).then(res=>{
                        if(res.code=='200'){
                            this.createDialogVisible = false;
